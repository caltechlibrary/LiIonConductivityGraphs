...
class MEGNetDescriptor:
    ...
    def _predict_structure(self, input_data: StructureOrMolecule | dict) -> np.ndarray:
        if isinstance(input_data, dict):
            # If the input is a graph represented as a dictionary
            inp = self.model.graph_converter.graph_to_input(input_data)
        else:
            # If the input is a pymatgen Structure or Molecule
            graph = self.model.graph_converter.convert(input_data)
    ...
    def _get_updated_prefix_level(self, prefix: str, level: int):
        mapping = {
            ...
            "concatenate": ["concatenate", 1 if level == 1 else "concatenate", None],
        }
        if self.version == "v2":
